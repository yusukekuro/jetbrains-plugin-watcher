...
```bash
jetbrains-plugin-watcher$ sam build
```
...
```bash
jetbrains-plugin-watcher$ sam local invoke WatcherFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
jetbrains-plugin-watcher$ sam local start-api
jetbrains-plugin-watcher$ curl http://localhost:3000/
```
...
```bash
jetbrains-plugin-watcher$ sam logs -n WatcherFunction --stack-name jetbrains-plugin-watcher --tail
```
